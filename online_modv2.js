(function () {
  ...
  // Добавление функциональности для открытия правого меню
  Lampa.Listener.follow('activity', function (e) {
    if (e.type === 'start' && e.component === 'full') {
      var button = e.body.find('.button--play');
      button.unbind('hover:enter').on('hover:enter', function () {
        var menu = $('<div class="menu-right"></div>');
        var trailer = $('<div class="menu-item">Трейлер</div>');
        var online = $('<div class="menu-item">Онлайн</div>');

        trailer.on('click', function () {
          // Логика для запуска трейлера
        });

        online.on('click', function () {
          // Логика для запуска онлайн модуля
          loadOnline(e.object);
        });
